...
export class UserAnalyticService {
  ...
  findAll(include: string[] = [], tx?: Prisma.TransactionClient) {
    return (tx || this.prisma).userAnalytic.findMany({
  ...
  async findOne(id: string, include: string[] = [], tx?: Prisma.TransactionClient) {
    const record = await (tx || this.prisma).userAnalytic.findUnique({
      where: { id },
      include: this.getIncludes(include),
    });
    if (!record) throw new NotFoundException(`UserAnalytic #${id} not found`);
    return record;
  }

  async create(dto: CreateUserAnalyticDto, tx?: Prisma.TransactionClient) {
    const data = { ...dto } as any;
    const result = await (tx || this.prisma).userAnalytic.create({ data });
    return result;
  }

  async update(id: string, dto: UpdateUserAnalyticDto, tx?: Prisma.TransactionClient) {
    const record = await this.findOne(id, [], tx);
    const data = { ...dto } as any;
    const result = await (tx || this.prisma).userAnalytic.update({ where: { id }, data });
    return result;
  }

  async remove(id: string, tx?: Prisma.TransactionClient) {
    const record = await this.findOne(id, [], tx);
    const result = await (tx || this.prisma).userAnalytic.delete({ where: { id } });
    return result;
  }
