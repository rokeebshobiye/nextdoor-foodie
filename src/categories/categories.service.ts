...
export class CategoryService {
  ...
  async findOne(id: string, include: string[] = [], tx?: Prisma.TransactionClient) {
    const record = await (tx || this.prisma).category.findUnique({
      where: { id },
      include: this.getIncludes(include),
    });
  ...
  async create(dto: CreateCategoryDto, tx?: Prisma.TransactionClient) {
    const data = { ...dto } as any;
    const result = await (tx || this.prisma).category.create({ data });
    return result;
  }

  async update(id: string, dto: UpdateCategoryDto, tx?: Prisma.TransactionClient) {
    const record = await this.findOne(id, [], tx);
    const data = { ...dto } as any;
    const result = await (tx || this.prisma).category.update({ where: { id }, data });
    return result;
  }

  async remove(id: string, tx?: Prisma.TransactionClient) {
    const record = await this.findOne(id, [], tx);
    const result = await (tx || this.prisma).category.delete({ where: { id } });
    return result;
  }
