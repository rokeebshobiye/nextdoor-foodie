...
export class PayoutAccountService {
  ...
  findAll(include: string[] = [], tx?: Prisma.TransactionClient) {
    return (tx || this.prisma).payoutAccount.findMany({
  ...
  async findOne(id: string, include: string[] = [], tx?: Prisma.TransactionClient) {
    const record = await (tx || this.prisma).payoutAccount.findUnique({
      where: { id },
      include: this.getIncludes(include),
    });
    if (!record) throw new NotFoundException(`PayoutAccount #${id} not found`);
    return record;
  }

  async create(dto: CreatePayoutAccountDto, tx?: Prisma.TransactionClient) {
    const data = { ...dto } as any;
    const result = await (tx || this.prisma).payoutAccount.create({ data });
    return result;
  }

  async update(id: string, dto: UpdatePayoutAccountDto, tx?: Prisma.TransactionClient) {
    const record = await this.findOne(id, [], tx);
    const data = { ...dto } as any;
    const result = await (tx || this.prisma).payoutAccount.update({ where: { id }, data });
    return result;
  }

  async remove(id: string, tx?: Prisma.TransactionClient) {
    const record = await this.findOne(id, [], tx);
    const result = await (tx || this.prisma).payoutAccount.delete({ where: { id } });
    return result;
  }
